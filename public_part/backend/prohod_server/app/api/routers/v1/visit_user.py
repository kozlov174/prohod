...
from app.api.schemas.visit import (
    RejectRequest, VisitRequestCreate, VisitRequestListUserResponse, VisitRequestUserResponse,
)
...
visit_user_router = APIRouter(prefix="/visit-requests-for-users", tags=["Visit-for-users"])


@visit_user_router.post(
    "/visit-requests/",
    response_model=dict,
)
...
@visit_user_router.get(
    "/visit-requests/",
    response_model=VisitRequestListUserResponse,
)
def get_visit_requests_by_status(
    ...
    if status:
        query = query.filter(VisitRequest.status == status).filter(VisitRequest.form.has(user_to_visit_id=user["user_id"])).options(joinedload(VisitRequest.form))
...
@visit_user_router.get(
    "/visit-requests/{id}",
    response_model=VisitRequestUserResponse,
)
def get_visit_request_by_id(
    ...
    del visit_request.who_processed_user_id
    del visit_request.who_processed_security_id
...
def accept_visit_request(
    id: str,
    current_user: dict = Depends(get_current_user),  # Предполагается, что здесь получаем текущего пользователя
    db: Session = Depends(get_session),
):
    request = db.query(VisitRequest).filter(VisitRequest.id == id).options(joinedload(VisitRequest.form)).first()
    if not request:
        raise HTTPException(
    ...
    if request.who_processed_user_id and request.who_processed_security_id:
        if request.status != VisitRequestStatusEnum.reject:
            ...
            sender.send_accept(id, email)

    db.commit()
    return {"message": "Request accepted"}


@visit_user_router.post("/visit-requests/{id}/reject", response_model=dict)
async def reject_visit_request(
    ...
    sender = EmailQrCodeSender()
    sender.send_reject(email,reject_data.rejection_reason)
