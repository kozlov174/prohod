...
def accept_visit_request(
    ...
    request = db.query(VisitRequest).filter(VisitRequest.id == id).first()
    if not request:
        raise HTTPException(
...
@visit_security_router.post("/visit-requests/{request_id}/reject", response_model=dict)
async def reject_visit_request(
    ...
    # Если хотя бы один из пользователей отклонил заявку, ставим статус rejected
