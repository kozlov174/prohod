...
export async function getVisitRequestsByStatus(status: VisitStatus) {
  return (await instance.get<GetVisitsResponse>(PagesURl.VISIT_FOR_SECURITY, { params: { status } })).data;
}

export async function getVisitRequestById(visitId: string) {
  return (await instance.get<VisitRequest>(PagesURl.VISIT_FOR_SECURITY + visitId)).data;
}

export async function acceptVisitRequest(visitId: string) {
  await instance.post(PagesURl.VISIT_FOR_SECURITY + visitId + '/accept');
}

export async function rejectVisitRequest(visitId: string) {
  await instance.post(PagesURl.VISIT_FOR_SECURITY + visitId + '/reject');
}
