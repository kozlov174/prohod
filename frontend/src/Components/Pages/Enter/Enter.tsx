...
function EnterComponent(): JSX.Element {
  ...
  useEffect(() => {
    if (!timer) {
      ...
      setSendedEmail(enterForm.getValues('emailToSendReply'));
      if (data) {
    ...
    const interval = setInterval(() => {
      setTimer(prev => {
        if (prev && prev > 0) {
          sessionStorage.setItem('timer', String(prev - 1));
          return prev - 1;
        }
        return undefined;
      });
  ...
  useEffect(() => {
    const data = sessionStorage.getItem('enter');
    if (data) {
      enterForm.reset(JSON.parse(data));
      getFileFromSessionStorage('passportPhoto').then(file => {
        if (file) {
          enterForm.setValue('passportPhoto', file);
        }
      });
    }
  }, []);
  ...
  useEffect(() => {
    if (enterForm.watch()) {
      sessionStorage.setItem('enter', JSON.stringify(enterForm.getValues()));
      saveFileToSessionStorage(enterForm.getValues('passportPhoto'), 'passportPhoto');
    }
  }, [enterForm.watch()]);
